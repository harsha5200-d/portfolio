# 🚀 Harsha Vardhan | Full Stack Java Developer

Welcome to my professional portfolio! I am a passionate Full Stack Developer specializing in building modern, scalable, and AI-powered web applications.

<div align="center">

[![Live Project](https://img.shields.io/badge/Live-Demo-brightgreen?style=for-the-badge&logo=google-chrome&logoColor=white)](https://harsha5200-d.github.io/portfolio/)
[![GitHub](https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/harsha5200-d)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/harsha-vardhan)

</div>

---

## 👨‍💻 About Me

I am a B.Tech student and a dedicated **Full Stack Java Developer** focused on creating intelligent systems that solve real-world problems. My expertise spans across frontend design, backend architecture, and AI/ML integrations.

- 📍 **Location:** Hyderabad, Telangana, India
- 🎓 **Education:** B.Tech (Computer Science)
- 💡 **Interests:** Machine Learning, Cloud Computing, UI/UX Design

---

## 🛠️ Tech Stack

| Category | Technologies |
| :--- | :--- |
| **Languages** | ![Java](https://img.shields.io/badge/-Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white) ![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white) ![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black) |
| **Frontend** | ![HTML5](https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white) ![Bootstrap](https://img.shields.io/badge/-Bootstrap-7952B3?style=flat-square&logo=bootstrap&logoColor=white) |
| **Backend** | ![Flask](https://img.shields.io/badge/-Flask-000000?style=flat-square&logo=flask&logoColor=white) ![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white) |
| **Databases** | ![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white) ![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white) |

---

## 🌟 Featured Projects

### 🏥 AI Powered Healthcare
An intelligent healthcare platform leveraging Machine Learning and Generative AI for disease prediction, report analysis, and personalized health insights.
- **Tech:** Python, Flask, ML, React

### ✈️ Airline Reservation System
A full-featured booking system with seat selection and passenger record handling.
- **Tech:** Java, Swing, SQL

### 🎓 AI Student Assistant
A smart learning platform designed to enhance student productivity through AI-driven guidance.
- **Tech:** JavaScript, AI APIs, Node.js

---

## 🚀 Installation & Setup

1. **Clone the repo**
   ```bash
   git clone https://github.com/harsha5200-d/portfolio.git
   ```
2. **Open the project**
   Simply open `index.html` in your browser.

---

## 📬 Let's Connect

Feel free to reach out for collaborations or just a friendly hello!

- **Email:** [harsha@example.com](mailto:harsha@example.com)
- **Portfolio:** [harsha5200-d.github.io/portfolio/](https://harsha5200-d.github.io/portfolio/)

---
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=100&section=footer" />
</p>

